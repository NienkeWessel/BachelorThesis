...
For all program instructions $S$, $S'$, lists $L$, $L'$, statements $s$, $s'$: if $\sos{S}{L}{s} \Rightarrow  \sos{S'}{L'}{s'}$ and $\cc{S}{L}{s} \to ^{*} \tr$, then $\cc{S}{L}{r} \to \cc{S'}{L'}{r'}$ for exactly one $r'$ related to $s'$. 
\end{lemma}
...
\begin{theorem}
For all statements $S$, $S'$, states $s$, $s'$: $\ns{S}{s}{s'}$ if and only if for some $L$ $\sos{S}{L}{s} \Rightarrow ^{*} \sos{\sk}{L}{s'}$ and $\cc{S}{L}{s} \to^{*} \tr$.
\end{theorem}
