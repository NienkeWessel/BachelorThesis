...
\begin{itemize}[noitemsep]
    ...
    \item  $S = \letm{x}{S}$ : assume $\ns{\letm{x}{S}}{s}{s'}$. Then the rule that has been aplied last must be [let$_{\textrm{ns}}$]. This has the following derivation tree \AxiomC{$\langle S, s[x\mapsto \perp] \rangle \to s''$}
\UnaryInfC{$\ns{\letm{x}{S}}{s}{s''[x \mapsto s(x)]}$}
\DisplayProof
