...
\begin{theorem}
\label{progressb}
If $\ccb{S}{L}{r}{M} ->^* \texttt{true}$, then $S = \texttt{skip}$ and $L = \texttt{Nil}$ or we have $\sosb{S}{L}{s}{M} \Rightarrow \sosb{S'}{L'}{s'}{M'}$ for some $S'$, $L'$, $s'$ and $M'$, and $s$ where $s$ is equivalent to $r$.
\end{theorem}
