...
\begin{proposition}
For every statement $S_1$, $S_2$, for every list $L$, for every state $s$, $s'$ and for every natural number $n$, if $\langle S_1, L, s \rangle \Rightarrow ^{n} \langle \texttt{\textrm{skip}}, L, s \rangle$, then $\langle S_1;S_2, L, s \rangle$ $\Rightarrow ^{n+2} \langle S_2, L, s \rangle$
\end{proposition}
