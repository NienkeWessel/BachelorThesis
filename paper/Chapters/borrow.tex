...
\begin{theorem}
\label{progressb}
For all program instructions $S$, reduced states $r$, lists $L$, and for all $\mathcal{M}$'s: if $\ccb{S}{L}{r}{M} ->^* \texttt{true}$, then $S = \texttt{skip}$ and $L = \texttt{Nil}$ or we have $\sosb{S}{L}{s}{M} \Rightarrow \sosb{S'}{L'}{s'}{M'}$ for some $S'$, $L'$, $s'$ and $M'$, and every $s$ related to $r$.
\end{theorem}
