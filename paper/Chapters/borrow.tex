...
\begin{theorem}
For every statement $S$, every state $s, s', s''$, every list $L$, if $\sosb{S}{L}{s}{M} \Rightarrow ^{*} \sosb{\texttt{skip}}{L}{s'}{M'}$ and $\sosb{S}{L}{s}{M} \Rightarrow ^{*} \sosb{\sk}{L}{s''}{M'}$, then $s' = s''$ and $\mathcal{M}' = \mathcal{M}''$.
\end{theorem}
...
\begin{theorem}
\label{preservationb}
For all statements $S$, $S'$, lists $L$, $L'$, states $s$, $s'$, reduced states $r$, and for all $\mathcal{M}$, $\mathcal{M'}$: if $\ccb{S}{L}{r}{M} \to ^* \texttt{true}$ and $\sosb{S}{L}{s}{M} \Rightarrow \sosb{S'}{L'}{s'}{M'}$ with $r$ related to $s$, then there is an $r'$ that is related to $s'$ and we have $\ccb{S'}{L'}{r'}{M'} \to ^* \texttt{true}$.
\end{theorem}
...
\begin{theorem}
\label{progressb}
For all statements $S$, reduced states $r$, lists $L$, and for all $\mathcal{M}$'s: if $\ccb{S}{L}{r}{M} ->^* \texttt{true}$, then $S = \texttt{skip}$ and $L = \texttt{Nil}$ or we have $\sosb{S}{L}{s}{M} \Rightarrow \sosb{S'}{L'}{s'}{M'}$ for some $S'$, $L'$, $s'$ and $M'$, and every $s$ related to $r$.
\end{theorem}
